...
export function mz() {
...

  {
    ...
    _ExtraBytesOffset = (offset: number) => {
      doshdr.splice(doshdr.length - 2, 2, ...asUint(offset, 16));
    };
    ...
    _HdrOffset = () => {
      doshdr.splice(doshdr.length - 2, 2, ...asUint(doshdr.length, 16));
    };

    //Minimum Alloc
    //Offset 0x0A
    _MinAlloc = (value: number) => {
      doshdr.splice(doshdr.length - 2, 2, ...asUint(value, 16));
    };
...
const denobin = new Uint8Array(mz().doshdr.concat(mz().pe_ext));
Deno.writeFileSync("bin/deno.doshdr.bin", denobin, {
  create: true,
  append: false,
});
